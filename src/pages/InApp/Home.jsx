...
function Home() {
  ...
  const { blogList, blogListLoading, blogListError } = useSelector(
    (state) => state.blog
  );
  console.log("Blogs: ", blogList);
  useEffect(() => {
    if (pathname === "/blog") dispatch(getAllBlogsFromDb());
  }, []);
  return (
    <div className="flex flex-col w-full flex-wrap py-4 font-poppins">
      <div className="h-full flex flex-row rounded overflow-hidden">
        <Carousel blogs={blogList?.slice(0, 4)} />
      </div>
      <h2 className="py-8 text-xl font-extrabold">Recents Posts</h2>
