...
import {
  getBlogFromDb,
  getAllBlogsFromDb,
} from "../../redux/actions/GetAllBlogs";
import BlogCard from "../../components/HomeBlogCard";
import {
  setEditBlogContent,
  deleteBlog,
} from "../../redux/actions/AddBlogActions";
...
function Blog() {
  ...
  return (
    <article className="flex flex-col md:flex-row gap-4 py-4 justify-stretch">
      ...
      <div
        className="flex flex-col items-start gap-4 font-poppins"
        style={{ flexBasis: "100%" }}
      >
        <div className="flex flex-row items-center justify-between w-full animate-form">
          {blog?.author && (
            <p className="text-sm font-bold">
              {new Date(blog?.createdAt).toString()}
            </p>
          )}
          {blog?.author &&
            ...
            !pathname.endsWith("/preview") && (
              <div className="relative flex gap-2">
                ...
                {showDel && (
                  <div className="absolute flex flex-col top-8 right-0 w-56 items-center z-10 bg-gray-100 rounded-lg shadow-lg overflow-hidden border">
                    <p className="p-4">Sure you want to delete the blog?</p>
                    <div className="flex border-t w-full text-center justify-between">
                      <button
                        onClick={handleDelete}
                        className="p-2 text-red-600 hover:text-white hover:bg-red-600 flex-1"
                      >
                        Delete
                      </button>
                      <button
                        onClick={() => setShowDel(false)}
                        className="p-2 hover:text-white hover:bg-black border-l flex-1"
                      >
                        Cancel
                      </button>
                    </div>
        ...
        <h2 className="text-5xl font-poppins font-extrabold animate-form">
          {blog?.title}
        </h2>
        ...
        <div className="flex flex-col md:flex-row w-full items-center justify-between gap-4">
          {blog?.author && (
            <>
              <p className="text-sm animate-form">
                - by,{" "}
                <span className="font-medium">Dr. {blog?.author?.name}</span>, @
              ...
              <ul className="md:w-full flex flex-row flex-wrap items-center justify-end gap-2">
                {shareLinks.map((link, index) => (
                  <a
                    ...
                    className="border flex items-center justify-center rounded-full p-2 hover:scale-125 transition-all"
                  >
                    {link.icon}
                  </a>
                ))}
        ...
        {blog?.author && !pathname.endsWith("/preview") && (
          <Link
            to={`/doctor/profile/${blog?.author?._id}`}
            className=" animate-form flex items-stretch gap-8 p-8 rounded w-full bg-gradient-to-b from-black/70 to-black/95 text-white hover:shadow-lg hover:cursor-pointer"
          >
            <div className="w-16 bg-gray-100 rounded-full overflow-hidden">
              <img
                draggable={false}
                className="w-full object-cover"
                src={blog?.author?.img || Avatar}
                style={{ aspectRatio: 1 / 1 }}
              />
            </div>
            <div className="flex flex-col justify-between">
              <p>
                {blog?.author?.role === "Doctor" && "Dr. "}
                {blog?.author?.name} |{" "}
                {blog?.author?.role === "Doctor" && blog?.author?.speciality}
              </p>
              <div className="flex items-center gap-1">
                ...
                <ArrowCircleRightIcon sx={{ fontSize: "20px" }} />
              </div>
            </div>
          </Link>
        )}
      ...
      {!pathname.endsWith("/preview") && blogList?.length > 1 && (
        <div
          className="font-poppins md:sticky top-16  border-t-2 md:border-l-2 md:border-t-0 pt-4"
          style={{ flexBasis: "20%" }}
        >
          <div className="flex flex-col md:sticky top-16 md:overflow-y-scroll md:max-h-[50em] md:px-4">
            <h3 className="border-b-4 border-red-600 font-bold bg-white md:sticky md:top-0">
              Latest Updates
            </h3>
            {blogList
