...
function Search() {
  ...
  return (
    <div
      className={`container w-full pt-4 px-4 md:px-0 font-poppins transition-all flex flex-col ${
        !searchOpen && "h-0 pt-0 p-0 opacity-0"
      }`}
    >
      <div className="flex flex-row gap-2 items-center justify-between">
        <form className="flex w-full gap-2" noValidate onSubmit={handleSearch}>
          <input
            ...
            placeholder={`Search ${
              pathname.includes("blog") ? "Blogs" : "Doctors"
            }`}
            className="p-2 w-full border-b-2 focus:outline-none focus:border-black transition-colors"
            required
          ></input>
          <button
            type="submit"
            className="rounded-full border border-black flex items-center justify-end w-max p-2 bg-black text-white"
            style={{ zIndex: 0 }}
          >
            <SearchIcon />
          </button>
        ...
        {result.data && (
          ...
          >
            {result.data.length > 0 && !showSearchResults ? (
              <ArrowDropDownIcon />
            ) : (
              <ArrowDropUpIcon />
            )}
        ...
        >
          ...
          {result.data &&
            result.data.map((item, idx) => (
              <li
                className="bg-transparent search"
                role={"button"}
