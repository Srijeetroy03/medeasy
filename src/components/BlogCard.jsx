    ...
    <div
      className={`flex ${
        pathname.includes("blog")
          ? "flex-col py-2 gap-1"
          : "flex-row p-4 gap-4 "
      } w-full items-stretch font-poppins border-t-2 border-gray-200`}
      key={blog._id}
    >
      <img
        draggable={false}
        src={blog?.image}
        className="w-60  bg-black rounded object-cover"
      />
      <div className="flex flex-col justify-evenly p-2">
        <h2 className="font-poppins font-extrabold text-2xl">{blog.title}</h2>
        ...
        <button
          onClick={handleBlog}
          className="font-bold text-sm hover:underline transition-all items-center gap-1 flex flex-row"
        >
          Read More
          <ArrowCircleRightIcon sx={{ fontSize: "12px" }} />
        </button>
      </div>
    </div>
  );
