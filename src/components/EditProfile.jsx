...
function EditProfile({ show, handleShow }) {
  ...
  return (
    ...
    userObj && (
      ...
      >
        <h2 className=" font-bold text-3xl container flex items-center justify-between">
          Update Your Profile{" "}
          <button className="self-end" onClick={handleShow}>
            <CancelIcon fontSize="lg" />
          </button>
        </h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 container">
          ...
          <div className="flex flex-col input-grp relative animate-form gap-1 delay-45">
            ...
            <p className="absolute bottom-0 right-0 p-2 text-xs">
              {userObj?.description?.length}{" "}
              {userObj?.description && "characters"} (Min. 100 characters)
            </p>
          </div>
          <div className="flex flex-col input-grp relative animate-form gap-1 delay-45">
            <label
              htmlFor="image"
              className="rounded-full flex self-start bg-gray-100 hover:outline-0 hover:border-b-2 hover:border-b-black transition-colors p-2"
            >
              <img
                className="w-40 rounded-full h-40 object-cover object-center"
                src={userObj?.img || Avatar}
              />
            </label>
            <input
          ...
          <button
            className="btn rounded-[8px]  bg-black text-white px-4 py-2 transition-all"
            type="submit"
          >
            Submit
          </button>
